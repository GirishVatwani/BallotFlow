...
// Function prototypes
bool vote(int voter, int rank, string choice);
void tabulate(void);
...
void tabulate(void)
{
    ...
    for (int i = 0; i < voter_count; i++)
    {
        for (int j = 0; j < candidate_count; j++)
        {
            voter_pref = preferences[i][j];
            if (!candidates[voter_pref].eliminated)
            {
...
bool print_winner(void)
{
    ...
    // Find Candidate with more than half of the votes
    for (int i = 0; i < candidate_count; i++)
    {
        if (candidates[i].votes > max_vote)
        {
            max_vote = candidates[i].votes;
            leading_candidate = candidates[i].name;
        }
    }

    if (max_vote >= half_votes)
    {
        printf("%s\n", leading_candidate);
        return true;
    }
